...
// SEARCH AND FILTER
var qs = '', _filters = [], _accommodation_type = [],  _activities = [], _amenities = [], _alert = [], _businessType = [], _businessStage = [], _cropType = [], _sectors = [], _features = [], _subFeatures = [], _media = [], _rules = [],
    _conditions = [], _winevarietals = [], _cuisines = [], _employmentType = [], _employmentCategory = [], _employmentStage = [], _employmentFunding = [],
    _jobType = [], _locRect = [];
...
//    Accommodation: {
//        accommodationType: [],
//        accommodationPriceMin: [],
//        accommodationPriceMax: [],
//        accommodationNightsMin: [],
//        accommodationNightsMax: [],
//        accommodationPets: [],
//        accommodationSmoking: []
//    }, 
//    AirQuality: {
//        AQIHigh: '',
//        AQILow: ''
...
function geoNavigator(xml) {
    ...
    if (iLocs >= 0) {
            ...
            switch (shapeType) {
                ...
                case "Centroid":
                    if (centroidLat != null && centroidLon != null) {
                        ...
                        currPin = new Microsoft.Maps.Pushpin(currLoc, { icon: currIcon, width: 70, height: 70, draggable: false, anchor: new Microsoft.Maps.Point(12, 12) });
                        var entity_id;
                        _allPoints.push(currLoc);
                        currPin.metadata = {
                            title: stitle,
        ...
        if ($(xml).find('item').size() >= 0) {
            ...
            map.setView({ zoom: _zoom, bounds: _bounds, animate: true });
            }
        }
    }
...
function parseCoord(sCoord) {
    ...
    var v = $.trim(sCoord.split(','));
    if (v.length > 1) {
        return new Microsoft.Maps.Location(parseFloat(v[1]), parseFloat(v[0]));
    }
...
function searchFilters(_filter, _val, _label, e) {
    if (e.checked) {
        switch (_filter) {
            case 'Activities':
                _activities.push(_val);
                break;
            case 'Amenities':
                _amenities.push(_val);
                break;
            case 'Alerts':
                _alerts.push(_val);
                break;
            case 'Conditions':
                _conditions.push(_val);
                break;
            case 'Cuisines':
                _cuisines.push(_val);
                break;
            case 'EmploymentType':
                _employmentType.push(_val);
                break;
        }
    }
}
